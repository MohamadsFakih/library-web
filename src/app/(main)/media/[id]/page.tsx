...
type CollectionEntry = { id: string; status: string; mediaId: string };
      ...
      <div className="card rounded-[var(--radius-xl)] overflow-hidden flex flex-col sm:flex-row">
        ...
        <div className="p-6 sm:p-8 flex-1 flex flex-col">
          <div className="flex items-start justify-between gap-3 mb-1">
            ...
            <span className="text-xs text-[var(--muted)]">{media._count.userMedia} tracking</span>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {media.genre && (
              <span className="px-2.5 py-1 rounded-full bg-[var(--surface)] text-xs font-medium text-[var(--muted)] border border-[var(--card-border)]">{media.genre}</span>
            )}
            {media.releaseDate && (
              <span className="px-2.5 py-1 rounded-full bg-[var(--surface)] text-xs font-medium text-[var(--muted)] border border-[var(--card-border)]">{new Date(media.releaseDate).getFullYear()}</span>
            )}
            {media.metadata && (
              <span className="px-2.5 py-1 rounded-full bg-[var(--surface)] text-xs font-medium text-[var(--muted)] border border-[var(--card-border)]">{media.metadata}</span>
            )}
          </div>
